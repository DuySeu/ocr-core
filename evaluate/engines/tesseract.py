"""Tesseract output as the evaluator's shape.

Written by ``core/serialize``, so the reading is the one in ``base``. This module
exists so a tesseract-specific quirk has somewhere to live without touching the
...
# Read every document the tesseract pipeline wrote under an output directory.
